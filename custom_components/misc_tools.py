...
async def insert_indicators_csv(
    ...
    csv_data: Annotated[str, Field(
        description='CSV content with indicators. First line must be headers. '
                    'Required columns: indicator, type. Optional: reputation, comment, expiration, severity. '
                    'Example: "indicator,type,reputation\\n1.2.3.4,ip,BAD\\nevil.com,domain,SUSPICIOUS"'
    )],
) -> str:
    """Inserts threat indicators (IOCs) via CSV file upload.
    The CSV must include a header row. Required columns: indicator, type.
    Supported types: ip, domain, hash, filename. Reputation: GOOD, SUSPICIOUS, BAD."""
    if not csv_data or not csv_data.strip():
        return create_response(data={"error": "csv_data is required — provide CSV content with header row"}, is_error=True)
    try:
        fetcher = await get_fetcher(ctx)
        result = await fetcher.send_request(
            "/public_api/v1/indicators/insert_csv",
            data={"request_data": csv_data},
        )
        return create_response(data=result)
    except PAPI_ERRORS as e:
        return create_response(data={"error": str(e)}, is_error=True)
    except Exception as e:
        logger.exception(f"insert_indicators_csv failed: {e}")
        return create_response(data={"error": str(e)}, is_error=True)

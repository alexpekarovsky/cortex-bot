...
async def get_assets(
    ...
    try:
        ...
        resp = await fetcher.send_request(
            path="/public_api/v1/assets/get_assets/",
            method="POST",
            data={"request_data": request_data or {}},
            omit_papi_prefix=True
        )
        return create_response(data=resp)
    except PAPIClientError as e:
        # API returns 404 "No asset found" when result set is empty
        if "No asset found" in str(e):
            return create_response(data={
                "reply": {"data": [], "metadata": {"filter_count": 0, "total_count": 0}},
                "success": "true",
                "note": "No assets matched the given filters. Use get_asset_by_id with a specific asset ID, "
                        "or get_endpoints for XDR-managed endpoints."
            })
        return create_response(data={"error": str(e)}, is_error=True)
    except PAPI_ERRORS as e:
        return create_response(data={"error": str(e)}, is_error=True)
    except Exception as e:
        logger.exception(f"get_assets failed: {e}")
        return create_response(data={"error": str(e)}, is_error=True)

...
async def update_case_timeline(
    ...
    The generated timeline is stored in the case's 'visualstoryline' custom field and can
            ...
            update_response = await fetcher.send_request(
                ...
                data={
                    "request_data": {
                        "incident_id": case_id,
                        "update_data": {
                            "visualstoryline": html_timeline
                        }
                    }
                }
            )
        except (PAPIServerError, PAPIClientError) as update_err:
            error_msg = str(update_err)
            # Extract valid field names from the API error to help the user
            if "Allowed keys" in error_msg:
                import re
                match = re.search(r"Allowed keys for update_data: \[([^\]]+)\]", error_msg)
                valid_fields = match.group(1) if match else "unknown"
                return create_response(data={
                    "error": f"Field 'visualstoryline' not found on case {case_id}.",
                    "valid_fields": valid_fields,
                    "hint": "The 'visualstoryline' custom field must exist on the case. "
                            "Create it in Settings > Objects Setup > Case Fields, or use one of the valid fields listed above.",
                    "timeline_generated": True,
                    "timeline_length": len(html_timeline),
                }, is_error=True)
            raise

        logger.info(f"Successfully updated case {case_id} with timeline")

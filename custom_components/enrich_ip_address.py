...
async def enrich_ip_address(
    ...
    **OPTION 1: GENERAL ENRICHMENT** (recommended for ad-hoc lookups)
    ─────────────────────────────────────────────────────────────────
    When you just want to check reputation of an IP without a specific case:

    1. Create a workspace: workspace = create_issue()
    2. Use the returned alert_id: enrich_ip_address(ip="8.8.8.8", alert_id=workspace["alert_id"])

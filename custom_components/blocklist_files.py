"""
Block List Files Tool

Adds file hashes to the Cortex XSIAM blocklist to prevent their execution
...
async def blocklist_files(
    ...
    This action cannot be easily reversed. Blocklisted files will be blocked from
    executing on all managed endpoints.

    Use this when:
    - You've identified malicious files that need immediate blocking
    - You want to prevent file execution across your environment
    - Adding known malware hashes to your enterprise blocklist
    - Performing rapid incident response to stop malware propagation

    Reversal:
    - Blocklisted files can be removed via the allowlist API if needed
    - The action is tracked and can be audited via incident_id

    Args:
        ctx: The FastMCP context.
        hash_list: List of file hashes to blocklist. Accepts MD5, SHA1, or SHA256.
                  Example: ["e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855", "d41d8cd98f00b204e9800998ecf8427e"]
        comment: Optional explanation for why these files are being blocklisted.
                Example: "Detected malware variant associated with incident #12345"
    ...
    if not confirm_destructive_action:
        return create_response(
            data={
                ...
                "message": "This is a HIGH risk action that blocklists files. "
                          "Set confirm_destructive_action=True to proceed.",
                "risk_level": "HIGH",
                "reversible": True,
                "reversal_method": "Files can be removed via allowlist API"
            },
            is_error=True
        )
    ...
    try:
        logger.info(f"Adding {len(hash_list)} hashes to blocklist")

        fetcher = await get_fetcher(ctx)

        # Build the blocklist request
        payload = {
            "request_data": {
                "hash_list": hash_list
            }
        ...
        result = {
            ...
            "status": "blocklist_initiated",
            "message": f"Successfully added {len(hash_list)} hash(es) to blocklist"
        }
        ...
        logger.info(f"Successfully blocklisted {len(hash_list)} hashes")
        ...
        logger.exception(f"PAPI error while blocklisting files: {e}")
        return create_response(
            data={
...
class BlocklistFilesModule(BaseModule):
    """
    Module for blocking malicious files across the environment.

    This module provides the blocklist_files tool that adds file hashes to the
    Cortex XSIAM blocklist, preventing them from executing on managed endpoints.

    Tools provided:
        - blocklist_files: Add file hashes to the enterprise blocklist
    """

    def register_tools(self):
        register_destructive(self, blocklist_files)

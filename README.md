# Cortex Bot — Community MCP Tools for Cortex XSIAM
...
> **Prerequisites:** You must install the [official Palo Alto Networks Cortex MCP Server](https://docs-cortex.paloaltonetworks.com/r/Cortex/Cortex-MCP-server/Create-custom-Cortex-MCP-server-tools) first. This repo adds community tools on top of it.
...
| **Response Actions** | 7 | Isolate endpoints, terminate processes, scan for malware, track actions |
| **File Operations** | 7 | Quarantine, restore, blocklist, allowlist, retrieve files |
| **Threat Hunting** | 7 | Run XQL queries, discover dataset schemas, enrich IPs/domains/files/URLs |
| **Detection Rules** | 4 | Create, search, get, delete XQL-based correlation rules |
...
| **Development Guides** | 12 | Pattern guides, building blocks, playbook generation, Slack workflows |
...
<details>
<summary><b>Threat Hunting (7 tools)</b></summary>

| Tool | Example Prompt |
|------|---------------|
| `run_xql_query` | "Hunt for PowerShell with bypass flag in the last 7 days" |
| `discover_dataset_schema` | "Show me the field names in the xdr_data dataset" |
...
<details>
<summary><b>Detection Rules (4 tools)</b></summary>

| Tool | Example Prompt |
|------|---------------|
| `insert_correlation_rule` | "Create a rule to detect SSH brute force over 10 failed logins" |
| `search_correlation_rules` | "List all my correlation rules" |
| `get_correlation_rule` | "Show me the details of correlation rule 48" |
| `delete_correlation_rule` | "Delete test correlation rule 46" |
...
<details>
<summary><b>Development Guides (12 tools — 9 pattern guides + building blocks + Slack + content guide)</b></summary>
...
<details>
<summary><b>IOC & BIOC Management (4 tools)</b></summary>

| Tool | Example Prompt |
|------|---------------|
| `get_indicators` | "Show me all IOC rules in my tenant" |
| `get_biocs` | "List all behavioral IOC rules" |
| `insert_bioc` | "Create a BIOC rule to detect lateral movement via WMI" |
| `get_datasets` | "List all available datasets for XQL queries" |
...
| Tool | Example Prompt |
|------|---------------|
| `get_audit_management_logs` | "Show me who made changes in XSIAM today" |
| `get_audit_agent_reports` | "Show me agent policy updates on Gaming" |
| `get_distributions` | "List all agent installation packages" |
| `get_endpoint_profiles` | "Show me endpoint security profiles and best practices" |
| `get_triage_presets` | "List forensic triage configurations" |
| `trigger_vulnerability_scan` | "Run a vulnerability scan on this asset" |

</details>

<details>
<summary><b>War Room & IOC Insert (4 tools)</b></summary>
...
9 tools require `demisto-sdk`: `sdk_validate`, `sdk_lint`, `sdk_upload`, `sdk_download`, `sdk_run`, `sdk_run_playbook`, `sdk_generate_docs`, `sdk_split`, `sdk_unify`. The other 97 tools work without it.
...
This repo provides `custom_components/` — community-built tools that plug into the official Palo Alto Networks Cortex MCP Server's custom components directory.
...
This project builds on top of the official [Palo Alto Networks Cortex MCP Server](https://docs-cortex.paloaltonetworks.com/r/Cortex/Cortex-MCP-server/Create-custom-Cortex-MCP-server-tools), which is an excellent foundation for AI-powered security operations. We are grateful to the Palo Alto Networks team for creating and maintaining the Cortex MCP Server and the Cortex XSIAM platform.

The base Cortex MCP Server is **not included** in this repository — you must install it separately from Palo Alto Networks.

**Trademarks:** Palo Alto Networks, Cortex, XSIAM, XSOAR, and XDR are trademarks of Palo Alto Networks, Inc. All other trademarks are the property of their respective owners.
